...
const App = () => {
    ...
    const [isLateView, setIsLateView] = useState(false) // Новое состояние
    ...
    const transformData = (data, isLate) => {
        ...
        data.forEach((item) => {
            ...
            // employees[fio][date] = String(value)
            employees[fio][date] = {
        ...
        const rowData = Object.keys(employees).map((fio) => {
            ...
            dates.forEach((date) => {
                ...
                const value =
                    dayData.day_status && dayData.day_status !== 'Явка'
                        ? dayData.day_status
        ...
        try {
            const updatedData = rowData.map((row) => {
                ...
                const updatedEmployeeData = {
                    ...employeeData,
                    skud_day_duration: {},
                    day_status: {}, // Добавляем поле day_status для обновления данных
                }
                ...
                Object.keys(row).forEach((key) => {
                    if (key !== 'fio' && key !== 'division' && key !== 'jobTitle') {
                        ...
                        if (dateKey) {
                            if (isNaN(row[key])) {
                                updatedEmployeeData.day_status[key] = row[key]
                            } else {
            ...
            //                     // Добавлено: сохранение строковых значений в skud_day_duration
            //                     updatedEmployeeData.skud_day_duration[key] = row[key]
